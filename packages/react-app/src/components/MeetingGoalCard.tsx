...
export function MeetingGoalCard({ sprintId }: { sprintId: number | null }) {
  ...
  const [collapsed, toggleCollapsed] = useCollapse("meetingGoal");
        ...
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-semibold text-foreground min-w-0 flex-1">
            <CollapseToggle collapsed={collapsed} onToggle={toggleCollapsed} className="w-full">
              <Megaphone className="h-3.5 w-3.5 text-primary shrink-0" aria-hidden="true" />
              Meeting goal
            </CollapseToggle>
          </h3>
          {sprintId !== null && !editing && !collapsed && (
            <button
              type="button"
              onClick={() => { setDraft(goal); setEditing(true); }}
              className="text-muted-foreground hover:text-foreground focus:outline-none focus:ring-1 focus:ring-ring rounded shrink-0"
